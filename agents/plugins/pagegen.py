...
## 调用 deploy_custom_checkin_page 的参数填法
工具签名：``deploy_custom_checkin_page(extra_requirements="")``。
工具会自动读本活动已上传的参考图作为视觉基准——所以你**不需要**也**不应该**
替图描述颜色。``extra_requirements`` 只装功能/结构性补充。
具体规则以工具自己的 docstring 为准（你已经能看到），下面是常见写法：
- 用户"按这张图设计" → ``extra_requirements=""``
- 用户"按图，去掉统计栏" → ``extra_requirements="去掉底部统计栏"``
- 用户没传图、用文字描述 → 把视觉风格写进 ``extra_requirements``
